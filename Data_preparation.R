...
setwd("~/Documents/OneDrive/RRR/ДИСЕР")
...
delete.vertices(allnet, isolates(allnet)) # delite all isolated vertices

# Setting the edge attributes - date, oblast and year
date <- as.Date(alldata$Event.start.date, format = "%d.%m.%Y")
set.edge.attribute(allnet, "date", alldata$Event.start.date) # точная дата начала протеста
set.edge.attribute(allnet, "oblast", alldata$Oblast.number) # номер области
set.edge.attribute(allnet, "year", alldata$Year) # год
